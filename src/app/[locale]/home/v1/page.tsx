...
export default function HomeV1() {
  ...
  const tools = [
    {
      ...
      accent: 'cyan',
      features: ['HEIC → JPG', 'PNG → WebP', 'Quality'],
    },
    {
      id: 'image-compressor',
    ...
    },
    {
      ...
      accent: 'amber',
      features: ['Video → GIF', 'GIF → MP4', 'Frames'],
    },
    {
  ...
  return (
    <div className="home-v1 min-h-full">
      {/* Hero Section */}
      <section className="relative overflow-hidden" style={{ background: 'radial-gradient(at 40% 20%, oklch(0.20 0.08 195 / 0.4) 0px, transparent 50%), radial-gradient(at 80% 0%, oklch(0.18 0.06 330 / 0.3) 0px, transparent 50%), radial-gradient(at 0% 50%, oklch(0.15 0.05 290 / 0.2) 0px, transparent 50%), oklch(0.08 0.01 240)' }}>
        {/* Grid Pattern */}
        ...
        {/* Floating Orbs */}
        <div className="absolute top-20 left-[10%] w-64 h-64 bg-[oklch(0.75_0.18_195/0.15)] rounded-full blur-[100px]" style={{ animation: 'home-v1-float 6s ease-in-out infinite' }} />
        <div className="absolute bottom-20 right-[15%] w-48 h-48 bg-[oklch(0.70_0.20_330/0.12)] rounded-full blur-[80px]" style={{ animation: 'home-v1-float 6s ease-in-out infinite', animationDelay: '-2s' }} />
              ...
              {features.map((feature, index) => {
                ...
                return (
                  <div
                    ...
                    className="group p-5 rounded-xl border border-[oklch(1_0_0/0.06)] bg-[oklch(0.12_0.015_250/0.5)] hover:border-[oklch(1_0_0/0.12)] hover:bg-[oklch(0.14_0.02_250/0.6)] transition-all duration-300 backdrop-blur-sm opacity-0"
                    style={{ animation: 'home-v1-fade-up 0.6s ease-out forwards', animationDelay: `${0.2 + index * 0.1}s` }}
                  >
      ...
      {/* Tools Grid */}
      <section id="tools" className="py-24" style={{ background: 'oklch(0.08 0.01 240)' }}>
            ...
            {tools.map((tool, index) => {
              ...
              return (
                <Link
                  key={tool.id}
                  href={tool.href}
                  className={`group relative p-6 rounded-2xl border bg-[oklch(0.10_0.015_250)] ${accent.border} ${accent.borderHover} ${accent.glow} transition-all duration-500 opacity-0`}
                  style={{ animation: 'home-v1-fade-up 0.6s ease-out forwards', animationDelay: `${0.1 + index * 0.05}s` }}
                >
                  {/* Gradient Overlay */}
                  ...
                  <div className="relative space-y-4">
                    {/* Icon */}
                    <div className={`inline-flex items-center justify-center w-12 h-12 rounded-xl ${accent.bg} ${accent.bgHover} transition-colors`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {tool.features.map((feature) => (
                        <span
                          key={feature}
                          className={`px-2.5 py-1 rounded-md text-xs font-medium ${accent.badge}`}
                        >
                          {feature}
                        </span>
                      ))}
      ...
      <section className="relative py-24 overflow-hidden">
        {/* Background */}
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, oklch(0.08 0.01 240), oklch(0.10 0.02 250), oklch(0.08 0.01 240))' }} />
      ...
      {/* Scoped Animations */}
      <style jsx global>{`
        @keyframes home-v1-float {
          0%, 100% { transform: translateY(0px); }
          50% { transform: translateY(-10px); }
        }
        @keyframes home-v1-fade-up {
